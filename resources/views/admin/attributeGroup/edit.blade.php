...
@section('content')
    ...
    <div class="col-md-10 pb-3 px-0">
        <div class="mb-4">
            ...
            <form class="customform p-3 bg-white" method="post" action="{{route('attributeGroup.update',$attributeGroup->id)}}">
                @method('PATCH')
                @csrf
                <div class="form-group row d-flex align-items-center">
                    <label for="name" class="custom-field-title col-sm-2 col-form-label text-right font-weight-bold mr-2">عنوان:</label>
                    <div class="col-sm-5">
                        <input type="text" class="custom-field form-control form-control-sm" id="name" name="name" value="{{$attributeGroup->name}}" >
                    </div>
                </div>
                <div class="form-group row d-flex align-items-center">
                    ...
                    <div class="col-sm-5">
                            <select name="type" class="custom-field form-control form-control-sm">
                                <option value="select" {{$attributeGroup->type == 'select' ? 'selected' : ''}}>لیست تکی</option>
                                <option value="multiple" {{$attributeGroup->type == 'multiple' ? 'selected' : ''}}>لیست چندتایی</option>
                            </select>
                    </div>
                </div>
